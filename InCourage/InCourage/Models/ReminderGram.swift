...
class ReminderGram {
    ...
    // MARK: - Initialization
    init(uid: String, sender: User, receiver: User, image: String, subject: String, message: String, loveRating: Int?) {
        self.uid = uid
        self.sender = sender
...
extension ReminderGram {
    ...
    convenience init?(reminderGramDictionary: [String : Any]) {
        guard let uid = reminderGramDictionary[ReminderGramKey.uid] as? String,
            let sender = reminderGramDictionary[ReminderGramKey.sender] as? User,
            let receiver = reminderGramDictionary[ReminderGramKey.receiver] as? User,
